...
public class PurchaseManagerPagingVO {
	...
	// PagingVO(12, 1, 5)
	public PurchaseManagerPagingVO(int total, int nowPage, int cntPerPage) { //12, 1, 10
		System.out.println("PurchaseManagerPagingVO의 total:" + total);
		setNowPage(nowPage); //1
		setTotal(total);//13
		setCntPerPage(cntPerPage);//10
		calcLastPage(total, cntPerPage); //calcLastPage(13, 10);
		calcStartEndPage(nowPage, cntPage); //calcStartEndPage(1, 5);
		calcStartEnd(nowPage, cntPerPage); //calcStartEnd(1, 10);
	}
	
	// 제일 마지막 페이지 계산
	public void calcLastPage(int total, int cntPerPage) { // 13,  10
	...
	public void calcStartEnd(int nowPage, int cntPerPage) { //1,10
